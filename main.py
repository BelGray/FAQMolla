...
{datetime.datetime.now()}""")
    ...
    try:
        if key == conf._secret_key:
            ...
            components.add_item(
                Button(style=disnake.ButtonStyle.secondary, label="English", custom_id="set_eng", emoji="🇬🇧"))
            components.add_item(
                Button(style=disnake.ButtonStyle.secondary, label="Russian", custom_id="set_ru", emoji="🇷🇺"))
            ...
            embed.add_field(name="10) Don't send links to sites that don't use SSL certificate",
                            value="`(nothing, but the bot will delete your message)`")
            components = disnake.ui.View()
            components.add_item(
                Button(style=ButtonStyle.secondary, label="Translate into Russian", custom_id="translate_rule_ru",
                       emoji="🇷🇺"))
...
async def on_button_click(interaction):
    ...
    if interaction.component.custom_id == "set_ru":
        ...
        try:
            ...
            await interaction.send("Отлично! Наслаждайся нашим сервером и познакомься с теплым русским комьюнити ;)",
                                   ephemeral=True)
        except:
            await interaction.send(":x: Something went wrong! Try again later :(", ephemeral=True)
